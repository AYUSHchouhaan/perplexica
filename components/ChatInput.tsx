...
export default function ChatInput({
  ...
  const [showNoMessagesModal, setShowNoMessagesModal] = useState(false);
  const showTerms = false;
  const { data: session } = useSession();
  const router = useRouter();
  const { messageCount, decreaseMessageCount } = useMessageCount();
  ...
  return (
  <div className="absolute bottom-4 left-0 right-0 z-10 w-full px-2 pointer-events-none">
      <div className="mx-auto w-full max-w-3xl text-center pointer-events-auto">
        ...
        <div
          className={`mt-2 rounded-[20px] border-reflect ${
            theme === "dark" ? "bg-[#251f2a]" : "bg-[#f5d5f5]"
          } p-2 backdrop-blur-lg`}
        >
          <form
            onSubmit={handleSubmit}
            className={`relative flex flex-col items-stretch gap-2 rounded-[20px] border ${
              theme === "dark" ? "!border-white/10 bg-[#2c2333]" : "!border-white/70 bg-[#fff0ff]"
            } px-3 text-secondary-foreground sm:max-w-3xl h-[7.2rem] max-sm:h-[8.0rem]`}
            ...
            <div className="mt-4 flex items-center justify-between">
              <div className="flex items-center space-x-2 pb-2 relative">
                ...
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    // Toggle web search enabled state
                    setWebSearchEnabled(!webSearchEnabled);
                  }}
                  title={webSearchEnabled ? 'Disable web search' : 'Enable web search'}
                  aria-pressed={webSearchEnabled}
                  className={`flex items-center gap-2 rounded-2xl px-2 py-1.5 border transition-colors disabled:text-secondary/40 disabled:cursor-not-allowed ${webSearchEnabled ? 'text-white' : '!border-white/10 hover:bg-black/20'}`}
                  style={webSearchEnabled ? (
                    theme === 'dark'
                      ? { background: 'radial-gradient(circle at center, #5e183d, #401020)' }
                      : { background: '#aa3067' }
                  ) : {}}
                >
              ...
              <div className="flex items-center space-x-2">
                <button
                  ...
                  disabled={!prompt.trim() || isLoading || messageCount <= 0}
                  className={`rounded-full p-1 text-white disabled:opacity-50 disabled:cursor-not-allowed h-8 w-8 flex items-center justify-center`}
                  style={
                    prompt.trim() && !isLoading && messageCount > 0
                      ? theme === 'dark'
                        ? { background: 'radial-gradient(circle at center, #5e183d, #401020)' }
                        : { background: '#aa3067' }
                      : {}
      ...
      <UpgradeModal
        isOpen={showNoMessagesModal}
        onClose={() => setShowNoMessagesModal(false)}
        theme={theme}
      />
    </div>
  );
