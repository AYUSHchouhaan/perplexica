...
export default function ChatArea({ 
  ...
  setWebSearchEnabled: externalSetWebSearchEnabled,
}: ChatAreaProps) {
  ...
  const actualWebSearchEnabled = externalWebSearchEnabled ?? webSearchEnabled;
  const actualSetWebSearchEnabled = externalSetWebSearchEnabled ?? setWebSearchEnabled;
  ...
  return (
  <main className={`firefox-scrollbar-margin-fix relative flex w-full h-full flex-col overflow-hidden transition-[width,height]`}>
  <div className={`absolute bottom-0 top-0 w-full overflow-hidden ${theme === 'dark' ? 'bg-[#211C26]' : 'bg-white'} bg-fixed pb-[140px] transition-all ease-snappy max-sm:border-none sm:translate-y-3.5`}>
      ...
      <div className="absolute bottom-0 left-0 right-0 w-full z-20 mt-4">
        {activeModel && onModelSelect && (
          <ChatInput 
            ...
            setWebSearchEnabled={actualSetWebSearchEnabled}
          />
        )}
      </div>
      ...
      {showScrollButton && (
        <div className="absolute bottom-[154px] inset-x-0 z-20 flex justify-center">
          <button
