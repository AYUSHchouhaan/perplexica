...
const SearchModal = ({ isOpen, onClose, theme }: SearchModalProps) => {
    ...
    const loadChats = useCallback(async () => {
        // useChats() already exposes chats and refreshChats; ensure chats are loaded
        if (status === 'loading') return;
        if (session?.user?.id) {
            try {
                await refreshChats();
            } catch (err) {
                console.error('Error refreshing chats from provider:', err);
            }
        }
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            const source = chats || [];
            const results = query
                ? source.filter(chat => chat.title.toLowerCase().includes(lowerCaseQuery))
                : source;
            const sorted = results.sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
            setFilteredChats(sorted.slice(0, 5)); // Limit results
        }
    }, [query, chats, isOpen]);
    ...
    const handleEnter = async (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Enter') {
            ...
            onClose();
            if (query.trim()) {
                // require authentication to create a chat
                if (!session?.user?.id) {
                    router.push('/auth');
                    return;
                }
                const res = await fetch('/api/chats', {
                    ...
                    body: JSON.stringify({ modelId: "gemini-1.5-flash" }),
                  });
